...
class BlockVpn extends SshVpn
{
    ...
    private function setCommand()
    {
        ...
        if (empty($user_chat_id)) {
            $this->command = "sudo /data/scripts/openvpncrm.sh off " . $this->getFileName();
        } else {
            $this->command = "sudo /data/scripts/openvpncrm.sh off " . $this->getFileName() . ' ' . $user_chat_id;
        }
    }
